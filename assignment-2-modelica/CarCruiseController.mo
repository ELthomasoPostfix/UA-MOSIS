package PCarCruiseController
  ...
  model CarCruiseController
    ...
    PCarCruiseController.CarAccToDisplacement lead_car annotation(
    ...
    PCarCruiseController.PIDController PID annotation(
  ...
  model CarAccToDisplacement
    ...
    parameter Modelica.Blocks.Interfaces.RealInput t annotation(
    ...
    parameter Modelica.Blocks.Interfaces.RealInput alt "The acceleration of the lead car in m/s^2" annotation(
  ...
  equation
    ...
    connect(xl0.y, sum1.u[1]) annotation(
      Line(points = {{-50, 80}, {30, 80}, {30, 32}}, color = {0, 0, 127}));
    connect(sum1.y, xlt) annotation(
      Line(points = {{54, 32}, {72, 32}}, color = {0, 0, 127}));
    connect(product.y, sum1.u[1]) annotation(
      Line(points = {{-16, 32}, {30, 32}}, color = {0, 0, 127}));
    connect(integrator2.y, sum1.u[1]) annotation(
      Line(points = {{18, -28}, {30, -28}, {30, 32}}, color = {0, 0, 127}));
    annotation(
      Diagram);
  ...
  equation
    ...
    connect(P.y, sum1.u[1]) annotation(
      Line(points = {{-38, 70}, {54, 70}, {54, 0}}, color = {0, 0, 127}));
    connect(I.y, sum1.u[1]) annotation(
      Line(points = {{30, 24}, {41, 24}, {41, 0}, {54, 0}}, color = {0, 0, 127}));
    connect(D.y, sum1.u[1]) annotation(
      Line(points = {{42, -70}, {54, -70}, {54, 0}}, color = {0, 0, 127}));
    connect(sum1.y, ut) annotation(
